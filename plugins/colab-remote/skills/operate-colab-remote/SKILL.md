...
- Use `mount_google_drive` to create the dedicated `MyDrive/codex-colab` workspace. On first use it may return `authorization_required=true` and open Google's approval page; ask the user only to approve it, then call the same tool again. Never ask for or handle an authorization code. Treat the returned `workspace_path` as the only Drive path agents may use. Never inspect, list, read, write, move, or delete its parent through `terminal_exec`, `execute_code`, a job, SSH, or notebook code.
