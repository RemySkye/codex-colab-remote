...
- Use `mount_google_drive` to create the dedicated `MyDrive/codex-colab` workspace. Treat the returned `workspace_path` as the only Drive path agents may use. Never inspect, list, read, write, move, or delete its parent through `terminal_exec`, `execute_code`, a job, SSH, or notebook code.
