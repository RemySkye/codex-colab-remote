...
One Python installer and one Python `colab-remote` user CLI contain the shared Windows, Linux, and macOS logic; the PowerShell and shell files only launch the installer. The user CLI manages updates, validated JSONC preferences, diagnostics, and plugin-owned credential aliases. On Linux/macOS the installed MCP server invokes the Colab CLI directly. On Windows the server runs natively but invokes the CLI inside WSL2 because Google does not provide a native Windows Colab CLI.
...
Local state lives in `~/.codex/colab-remote` by default. It contains configuration, job/lease metadata, notifications, and transfer checkpoints. The directory is restricted to the current user. Colab OAuth credentials remain in the CLI's separate configuration directory and are never parsed by the server.
...
- `keyring` for Windows Credential Manager, macOS Keychain, and Linux Freedesktop Secret Service integration
- `mcp>=1.20,<2`
